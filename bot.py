...
bot = telebot.TeleBot('5063505580:AAEuwlJcYtZUw66pzEDSKqc1bIqSB-271Ao')
...
def start(message: types.Message):
    bot.send_message(message.chat.id, 'Anonim Sohbet Botu!\nSohbet Başlatmak: /bul\nSohbet Sonlandırmak: /dur\n\nCoder: @yusiqo')
...
def find(message: types.Message):
    ...
    if message.chat.id not in users:
        bot.send_message(message.chat.id, 'Aranıyor...')
        ...
        if freeid is None:
            ...
            bot.send_message(message.chat.id, 'Bulundu!')
            bot.send_message(freeid, 'Bulundu!')
...
def stop(message: types.Message):
    ...
    if message.chat.id in users:
        bot.send_message(message.chat.id, 'Duruyor...')
        bot.send_message(users[message.chat.id], 'Karşı Taraf Durdurdu...')
...
def chatting(message: types.Message):
    if message.chat.id in users:
        ...
        bot.send_message(message.chat.id, 'kimse seni duyamaz...')
